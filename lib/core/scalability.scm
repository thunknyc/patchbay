(define-module (core scalability)
  ...
  #:replace (socket bind connect shutdown recv send)
  #:export (bus-socket
            pair-socket
...
(define (bind s addr)
  (let ((result (nn-bind s (string->pointer addr))))
    (>= result 0)))
...
(define (connect s addr)
  (let ((result (nn-connect s (string->pointer addr))))
    (>= result 0)))
...
(define (call-with-recv-msg s proc)
  (let ((msg (recv s)))
    (if msg
        (let ((value (proc msg)))
          (freemsg msg)
...
(define (call-with-recv-string s proc)
  (call-with-recv-msg s
    (lambda (msg)
